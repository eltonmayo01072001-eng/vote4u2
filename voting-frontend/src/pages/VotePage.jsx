...
export default function VotePage() {
  ...
  const [votingEndedText, setVotingEndedText] = useState("Voting ended");
  ...
  useEffect(() => { 
    fetchVote();
    // Pre-translate "Voting ended"
    translateText("Voting ended").then(t => setVotingEndedText(t));
  }, [id]);
  ...
  const handleSubmit = async () => {
    if (!selected.length) {
      alert(await translateText("Select at least one option"));
      return;
    }
    try {
  ...
  if (!vote) return <p className="text-center mt-10 text-gray-600">{await translateText("Loading...")}</p>;
    ...
    <div className="max-w-2xl mx-auto p-6 mt-10 bg-gray-50 shadow-lg rounded-xl">
      ...
      {(showResults || hasVoted) && (
        <div className="bg-white p-4 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold mb-3 text-center text-gray-800">{await translateText("Results")}</h2>
          {translatedOptions.map((opt, idx) => (
            <div key={opt} className="flex justify-between mb-2 p-2 border-b border-gray-200">
              <span className="text-gray-700">{opt}</span>
              <span className="font-semibold text-gray-800">{optionCounts[idx]} {await translateText("votes")}</span>
            </div>
          ))}
        </div>
      )}
